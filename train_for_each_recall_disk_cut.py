...
for i, file in enumerate(files):
    ...
    subprocess.run(f'python3 train_high_recall.py TRAIN --output {output_dir} --model tf_efficientnetv2_s_in21ft1k --pretrained -b 32 -w0 60.0 -w1 40.0', shell=True)
    files = glob.glob(f'{output_dir}/20*/*')
    for file in files:
